...
import { ArrowLeft, MapPin, Tag, Send, Lock } from 'lucide-react';
import { useCartStore } from '@/lib/store';
import { formatPrice } from '@/lib/utils';

export default function CartPage() {
  const router = useRouter();
  const { items, tableNumber, outlet, updateQuantity, removeItem,
          subtotal, serviceCharge, tax, total } = useCartStore();
        ...
        <div className="phone-shell">
          <div className="flex-1 flex flex-col items-center justify-center px-6 pb-10">
            <div className="relative w-[100px] h-[100px] mb-6">
              ...
              <div className="absolute inset-0 rounded-full bg-gold-400/12 flex items-center justify-center text-4xl">
                ✓
              </div>
            </div>
            <h2 className="font-serif text-[26px] text-[#f5e9d0] font-semibold mb-2 text-center">Order Placed!</h2>
            <p className="text-[14px] text-white/35 text-center leading-relaxed mb-8 px-4">
            ...
            <div className="bg-gold-400/10 border border-gold-400/25 rounded-full px-5 py-2 mb-8">
              <span className="text-[13px] text-gold-400 font-medium">Order #LM-2847</span>
            </div>
            <button
              onClick={() => router.push('/guest/tracking')}
              className="btn-gold max-w-[280px]"
            >
              📡 Track My Order
            </button>
          </div>
      ...
      <div className="phone-shell">
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide px-5">
          ...
          <div className="flex flex-col gap-2 mb-4">
            {items.length === 0 && (
              <p className="text-white/25 text-sm text-center py-8">Your cart is empty</p>
            )}
            {items.map((item) => (
              <div key={item.id} className="flex items-start gap-3 p-3.5 rounded-2xl bg-white/[0.025] border border-white/[0.05]">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center border border-white/[0.08] rounded-[10px] w-fit overflow-hidden">
                    <button onClick={() => updateQuantity(item.id, item.quantity - 1)}
                      className="w-[30px] h-7 bg-white/[0.04] hover:bg-gold-400/10 flex items-center justify-center text-white/40 text-sm transition-colors">
                      −
                    </button>
                    ...
                    <button onClick={() => updateQuantity(item.id, item.quantity + 1)}
                      className="w-[30px] h-7 bg-white/[0.04] hover:bg-gold-400/10 flex items-center justify-center text-gold-400 text-sm transition-colors">
                      +
                    </button>
                  </div>
                </div>
                <div className="flex flex-col items-end gap-2 flex-shrink-0">
                  ...
                  <button onClick={() => removeItem(item.id)}
                    className="w-7 h-7 rounded-[8px] bg-red-500/[0.07] border border-red-500/15 flex items-center justify-center text-red-400/70 hover:bg-red-500/15 transition-colors text-xs">
                    ✕
                  </button>
                </div>
              </div>
          ...
          <div className="flex items-center gap-2.5 p-3 rounded-[14px] bg-gold-400/[0.05] border border-gold-400/15 mb-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-[14px] font-medium text-[#f5e9d0]">
                Table {tableNumber} · {outlet} · Ground Floor
              </p>
            </div>
          ...
          <textarea
            ...
            rows={2}
            className="w-full rounded-[14px] p-3 text-[13px] mb-4 resize-none"
          />
