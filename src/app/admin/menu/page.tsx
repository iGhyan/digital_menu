  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.05] bg-surface-400">
        <div>
          <h1 className="font-serif text-[20px] text-[#f5e9d0] font-semibold">
            Menu Management
          </h1>
          <p className="text-[12px] text-white/25 mt-0.5">
            La Maison · Main Hall · Last synced 2 min ago
          </p>
        </div>
      ...
      <div className="flex-1 p-6 overflow-y-auto">
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          {STATS.map((s) => (
            <div
              key={s.label}
              className="bg-surface-100 border border-white/[0.06] rounded-[14px] p-4"
            >
              <p className="text-[11px] text-white/25 uppercase tracking-widest mb-2">
                {s.label}
              </p>
              <p className="font-serif text-[26px] text-[#f5e9d0] font-semibold mb-1">
                {s.val}
              </p>
              <p className={`text-[11px] flex items-center gap-1 ${
                s.up === true ? 'text-green-400' : s.up === false ? 'text-red-400' : 'text-white/25'
              }`}>
          ...
          {['All Items', 'Categories', 'Inactive', 'Drafts'].map((t, i) => (
            <button
              ...
              className={`px-5 h-8 rounded-[9px] text-[12px] transition-all ${
                i === 0
                  ? 'bg-surface-100 border border-white/[0.08] text-[#f5e9d0] font-medium'
        ...
        <div className="bg-surface-100 border border-white/[0.06] rounded-2xl overflow-hidden">
          ...
          {/* Head */}
          <div
            className="grid gap-3 px-4 py-3 border-b border-white/[0.05]"
            style={{ gridTemplateColumns: '40px 1fr 120px 80px 80px 90px 80px' }}
          >
            {['', 'Item', 'Category', 'Price', 'Orders', 'Status', 'Actions'].map((h) => (
              <div
                key={h}
                className="text-[11px] text-white/20 uppercase tracking-widest font-medium"
              >
                {h}
              </div>
            ))}
          ...
          {filtered.map((item) => (
            ...
            >
              ...
              <div className="font-serif text-[13px] text-gold-400 font-semibold">
          ...
          <div className="flex items-center justify-between px-4 py-3.5 border-t border-white/[0.05]">
            <p className="text-[12px] text-white/20">
              Showing {filtered.length} of 84 items
            </p>
      ...
      {modal.open && (
        ...
        >
          <div className="bg-surface-100 border border-white/[0.08] rounded-[20px] w-[420px] max-h-[85vh] overflow-y-auto p-6">
            ...
            {/* Name */}
            <div className="mb-4">
              <label className="block text-[11px] text-white/30 uppercase tracking-widest mb-1.5">
                Item Name
              </label>
              <input
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                ...
                <select
                  defaultValue={modal.item?.category}
                  className="w-full h-10 text-[13px] bg-white/[0.04] border border-white/[0.08] rounded-xl px-3"
                >
            ...
            <div className="mb-4">
              ...
              <label className="flex flex-col items-center gap-2 p-6 rounded-xl border border-dashed border-white/10 bg-white/[0.02] cursor-pointer hover:border-gold-400/35 hover:bg-gold-400/[0.04] transition-all">
                ...
                <CloudUpload
                  size={28}
                  className={uploadName ? 'text-gold-400' : 'text-white/15'}
                />
                ...
                <span className="text-[10px] text-white/12">
                  PNG, JPG up to 5MB · Direct S3 presigned upload
                </span>
              </label>
            </div>
